...
function RolesSection() {
  ...
  return (
    <div
      className="w-full relative mx-auto flex flex-row items-center justify-center"
      id="about"
    >
      <BackgroundBeams className="pb-2 sm:py-20 bg-gray-900"></BackgroundBeams>
      <ContentContainer>
        <motion.div
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 xl:gap-10">
          {config.roles.roles.map((role, index) => (
            ...
            >
              <BackgroundGradient
                className={`rounded-[22px] max-w-sm p-4 sm:p-10 bg-black/50 min-h-full`}
                key={role.title}
              >
                <RoleIcon
                  icon={role.image}
                ...
                <p className="text-sm xl:text-base text-gray-100 leading-6">
                  {role.description}
                </p>
              </BackgroundGradient>
            </motion.div>
